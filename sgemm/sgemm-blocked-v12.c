...
void Packing_A(float *src, float *dst, int lda, int M, int K){
...
void Packing_B(float *src, float *dst, int ldb, int K, int N){
...
void Packing_B_Test(float *src, float *dst, int ldb, int K, int N){
    //dim_first: K, dim_second: N
    float *tosrc,*todst;
    todst=dst;
    tosrc=src;
...
{ 
  ...
  // if((M % SMALL_BLOCK_M_64_SIZE == 0) && (N % SMALL_BLOCK_N_4_SIZE == 0))
  // {
  //   for (int j = 0; j < N; j += SMALL_BLOCK_N_4_SIZE) 
...
void square_sgemm (int lda, float* __restrict__ A, float* __restrict__ B, float* __restrict__ C)
{
  if(lda > 130)
  {
    ...
    {
      // float* B_TEST = (float*)_mm_malloc(sizeof(float) * lda * lda, 256);
      // Packing_B_Test(B, B_TEST, lda, lda, lda);
      ...
      for (int k = 0; k < lda; k += BLOCK_K_SIZE)
      {
        ...
        // Packing B
        Packing_B(B + k + j*lda, B_BUFF, lda, K, N);
        ...
        for (int i = 0; i < lda; i += BLOCK_M_SIZE)
        {
          ...
          // Packing A
          Packing_A(A + i + k*lda, A_BUFF, lda, M, K);
          /* Perform individual block sgemm */
          do_block_large(BLOCK_M_SIZE,lda,lda, M, N, K, A_BUFF, B + k + j*lda, C + i + j*lda);
        }
      }
    }
    _mm_free(A_BUFF);
